...
static void mass_mem_cfg_gpio(void)
{

	s3c_gpio_cfgpin(GPIO_MASSMEMORY_EN, S3C_GPIO_OUTPUT);
	s3c_gpio_setpull(GPIO_MASSMEMORY_EN, S3C_GPIO_PULL_UP);

	s3c_gpio_cfgpin(GPIO_MASSMEMORY_EN2, S3C_GPIO_OUTPUT);
	s3c_gpio_setpull(GPIO_MASSMEMORY_EN2, S3C_GPIO_PULL_UP);

	printk("\nA1#######\n GPIO_MASSMEMORY_EN %x GPIO_MASSMEMORY_EN2 %x", gpio_get_value(GPIO_MASSMEMORY_EN), gpio_get_value(GPIO_MASSMEMORY_EN2));

}

void mass_mem_ldo_en(bool onoff)
{
	if (onoff)
	{
...
late_initcall(max8893_pmic_init);			//NAGSM_Android_SEL_Kernel_Aakash_20101005
