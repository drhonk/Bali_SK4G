...
static int m5mo_set_zero_step(struct i2c_client *client)
{
	int i, err;
	u32 status = 0;	
	cam_dbg("E\n");
...
retry:
	...
	case FOCUS_MODE_FD_DEFAULT:
		mode = 0x00; //0x03; //NAGSM_ANDROID_HQ_CAMERA_SoojinKim_20110105
		break;
...
retry:
	switch (val) {
		...
		CHECK_ERR(err);		
		err = m5mo_writeb(client, M5MO_CATEGORY_CAPPARM, M5MO_CAPPARM_WDR_EN, 0x02);
		CHECK_ERR(err);
		break;
	default:
		cam_warn("invalid value, %d\n", val);
...
static int m5mo_s_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	case V4L2_CID_CAMERA_VINTAGE_MODE: 
		err = m5mo_set_vintage_mode(sd, ctrl->value);
		break;
