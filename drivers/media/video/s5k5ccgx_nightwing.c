...
static int s5k5ccgx_get_iso(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	if (read_value <= 384 ) ctrl->value=50;
...
static int s5k5ccgx_s_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	switch (ctrl->id) 
	{
		...
		case V4L2_CID_CAMERA_SET_FLASH:
			s5k5ccgx_msg(&client->dev, "%s: V4L2_CID_CAMERA_SET_FLASH \n", __func__);
			#ifdef S5K5CCGX_FLASH_SUPPORT
...
static int s5k5ccgx_check_dataline_stop(struct v4l2_subdev *sd)
{
	...
	//NAGSM_HQ_CAMERA_LEESUNGKOO_20110323 : It should be reset before starting preview.
	err = s5k5ccgx_reset(sd);
	if(err < 0){
		dev_err(&client->dev, "%s: failed: reset\n", __func__);
		return -EIO;
	}	
	mdelay(100);
	//NAGSM_HQ_CAMERA_LEESUNGKOO_20110323 : It should be reset before starting preview.
...
static int s5k5ccgx_init(struct v4l2_subdev *sd, u32 val)
{
	...
	if(state->vt_mode == 0)
	{
		...
		s5k5ccgx_msg(&client->dev, "%s: camera initialization(S5K5CCGX_INIT_SET)\n",__func__);
	if (err < 0) {
		//This is preview fail 
		s5k5ccgx_msg(&client->dev, "%s: camera initialization failed. err(%d)\n", \
	...
	else
	{
		...
		s5k5ccgx_msg(&client->dev, "%s: camera initialization(S5K5CCGX_INIT_VT_SET)\n",__func__);
		if (err < 0) {
			//This is preview fail 
