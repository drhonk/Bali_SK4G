...
static void InitCBusRegs( void )
{
	...
	I2C_WriteByte(SA_TX_CBUS_Primary, 0x89, 0);										// not for source
	I2C_WriteByte(SA_TX_CBUS_Primary, 0x8A, MHL_FEATURE_RCP_SUPPORT | MHL_FEATURE_RAP_SUPPORT);
	...
	I2C_WriteByte(SA_TX_CBUS_Primary, 0x8E, MHL_INT_AND_STATUS_SIZE);
...
{	
    ...
    DelayMS(T_SRC_WAKE_PULSE_WIDTH_1 + 1);	// adjust for code path
	...
	I2C_WriteByte(SA_TX_Page0_Primary, 0x96, (I2C_ReadByte(SA_TX_Page0_Primary, 0x96) & 0x3F));
    DelayMS(T_SRC_WAKE_PULSE_WIDTH_2 + 6);	// adjust for code path
...
void	ProcessRgnd( void )
{
	...
	else
	{		mhl_cable_status |= MHL_1K_IMPEDANCE_VERIFIED;
		...
		//
		// Setup for MHL discovery
		//
		SET_BIT(SA_TX_Page0_Primary, 0x95, 5);
	}
...
bool SiiMhlTxRcpkSend( void )
{
	return	( MhlTxSendMscMsg ( MHL_MSC_MSG_RCPK, 0 ) );
}
