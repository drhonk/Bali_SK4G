...
#include <media/camsensor_type.h> //NAGSM_HQ_CAMERA_LEESUNGKOO_20101231 : to support two type front camera
...
int gcamera_sensor_back_type = CAMERA_SENSOR_ID_BACK_ORG;
int gcamera_sensor_back_checked = false; 
int gcamera_sensor_front_type = CAMERA_SENSOR_ID_FRONT_ORG;
int gcamera_sensor_front_checked = false; 
//NAGSM_HQ_CAMERA_LEESUNGKOO_20101231 : to support two type front camera
...
static int fimc_camera_init(struct fimc_control *ctrl)
{
	...
	/* enable camera power if needed *///NAGSM_HQ_CAMERA_LEESUNGKOO_20101231 : to support two type front camera
	...
	if (ret < 0) { //NAGSM_HQ_CAMERA_LEESUNGKOO_20101231 : to support two type front camera
		gcamera_sensor_back_checked = false;
		gcamera_sensor_front_checked = false;		
	}
	if (ret == -ENOIOCTLCMD) {
...
int fimc_release_subdev(struct fimc_control *ctrl)
{
	...
	if(ctrl && ctrl->cam && ctrl->cam->sd){
		...
		ctrl->cam->sd = NULL;
		if(ctrl->cam->cam_power && ctrl->device_onoff)//NAGSM_HQ_CAMERA_LEESUNGKOO_20101231 : to support two type front camera
		{
...
//NAGSM_HQ_CAMERA_LEESUNGKOO_20101231 : to support two type front camera
	...
	if (i == 3000)	
	{
		return ((gcamera_sensor_front_checked&0xFF)<<8)|(gcamera_sensor_front_type&0xFF); 
	}	
//NAGSM_HQ_CAMERA_LEESUNGKOO_20101231 : to support two type front camera
...
//NAGSM_HQ_CAMERA_LEESUNGKOO_20101231 : to support two type front camera
	fimc_dbg("%s i=%d, checked (%d) , id (%d)",__func__  , i,  gcamera_sensor_front_checked , gcamera_sensor_front_type );
	if ((i==1 || i >= CAMERA_SENSOR_ID_FRONT_START && i < CAMERA_SENSOR_ID_FRONT_END )&& !gcamera_sensor_front_checked)
	{
		...
		v_ctrl.id = V4L2_CID_CAM_FRONT_SENSOR_TYPE;
		err = ctrl->cam->sd->ops->core->g_ctrl(ctrl->cam->sd,&v_ctrl);
		if (err >= 0)
		{
			fimc_dbg("%s checkd sensor type = %d\n", __func__,i);		
			gcamera_sensor_front_type = i;
...
int fimc_streamon_capture(void *fh)
{
	...
	int rot;
	int ret;
#if 1 //NAGSM_HQ_CAMERA_LEESUNGKOO_20101231 : to support two type front camera
	switch (gcamera_sensor_front_type)
	{
		case CAMERA_SENSOR_ID_FRONT_S5K6AAFX:
			{
				char *s5k6aafx = "S5K6AAFX 16-003c";
				device_id = strcmp(ctrl->cam->sd->name, s5k6aafx);
			}
			break;
		case CAMERA_SENSOR_ID_FRONT_SR130PC10:
			{
				char *sr130pc10 = "SR130PC10 16-0020"; //NAGSM_HQ_CAMERA_LEESUNGKOO_20110103
				device_id = strcmp(ctrl->cam->sd->name, sr130pc10);
			}
			break;
	}
#else
	...
	if (ctrl->id != 2) {
		...
		ret = subdev_call(ctrl, video, s_stream, 3);
	}
...
int fimc_streamoff_capture(void *fh)
{
	...
	if (ctrl->id != 2) {
		...
		v4l2_subdev_call(ctrl->cam->sd, video, s_stream, 4);
	}
