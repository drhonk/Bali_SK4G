...
static u8 fsa9480_device1 = 0, fsa9480_device2 = 0, fsa9480_adc = 0, fsa9480_interrupt1 =0;
int usb_path = 0;
...
#elif CONFIG_S5PC110_SIDEKICK_BOARD
		if (HWREV == 0x0d)	// TI mUSB Switch
		{
			if(FSA9480_Get_USB_Status()) 
				fsa9480_write(client, REGISTER_MANUALSW1, 0x49);  // D+/- switching by Audio_L/R 
			else	
				fsa9480_write(client, REGISTER_MANUALSW1, 0x00);
		}else{
		fsa9480_write(client, REGISTER_MANUALSW1, 0x48);  // D+/- switching by Audio_L/R 
		}	
#else
...
void FSA9480_ReadIntRegister(struct work_struct *work)
{
	...
	{
		msleep(5);
		fsa9480_read(client, REGISTER_CARKITINT1, &carkit1);
		
		msleep(5);
		fsa9480_read(client, REGISTER_CARKITINT2, &carkit2);
	}	
#endif

	usb_state = (device2 << 8) | (device1 << 0);

	printk("[FSA9480] %s : interrupt1 : 0x%x, interrupt2 : 0x%x, device1 : 0x%x, device2 : 0x%x\n",
		__func__, interrupt1, interrupt2, device1, device2);
...
u8 FSA9480_Get_I2C_USB_Status(void)
{
	...
	if( (device2 & FSA9480_DEV_TY2_AV) || (device1 == FSA9480_DEV_TY1_USB) ||(device2== FSA9480_DEV_TY2_JIG_USB_ON) ||(device2 == FSA9480_DEV_TY2_JIG_USB_OFF) )
		{
		   return 1;
		}
	else
...
void connectivity_switching_init(struct work_struct *ignored)
{
	...
	if((switch_sel == 1) || (factoryresetstatus == 0xAE)){
		...
		if(lpm_mode_check){	// boot on charge mode
            ...
            iTemp = iTemp & 0x3F;  // disable Fast charging Timer
			ap_usb_power_on(0);
