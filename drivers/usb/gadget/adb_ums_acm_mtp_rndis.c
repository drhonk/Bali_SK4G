...
#elif defined(CONFIG_S5PC110_SIDEKICK_BOARD)  //Config option set on T959 & Sidekick 
...
recheck:
	...
	currentusbstatus=enable;
		...
		{
			extern struct switch_dev indicator_dev;
			extern int connectivity_indicator_init_state;
			...
			if(indicator_dev.state == 0 && prev_enable_status == USBSTATUS_ASKON && connectivity_indicator_init_state == 1)
			{
