...
#define HALL_DEBUG 0
#define HALL_INT S5PV210_GPH1(5)	
...
#endif
	cur_val = gpio_get_value(HALL_INT);
...
static void hall_int_func(struct work_struct *work)
{
	...
	for(check_count = 0; check_count <3; check_count++)
	{
		...
		check_hall_ic_state = gpio_get_value(HALL_INT);
...
static int __init hall_probe(struct platform_device *pdev)
{
	...
	ktime_t hall_polling_time;
	s3c_gpio_cfgpin(HALL_INT, S3C_GPIO_INPUT);
	s3c_gpio_setpull(HALL_INT, S3C_GPIO_PULL_NONE);
	...
	s3c_gpio_cfgpin(HALL_INT, S3C_GPIO_SFN(3));
	s3c_gpio_setpull(HALL_INT, S3C_GPIO_PULL_NONE);
	set_irq_type(IRQ_EINT13, IRQ_TYPE_EDGE_BOTH);
	//set_irq_type(HALL_INT, IRQ_TYPE_EDGE_BOTH);

	ret = request_irq(IRQ_EINT13,hall_func,IRQF_DISABLED, DEVICE_NAME, hall);
	if(ret){
