...
unsigned int fg_read_soc(void)
{
	...
	// if(ce_for_fuelgauge){
	//	 FGAdjustSOC = ((FGPureSOC - 130)*10000)/9720; // (FGPureSOC-EMPTY(1.2))/(FULL-EMPTY(?))*100
		FGAdjustSOC = ((FGPureSOC - 60)*10000)/9420; // (FGPureSOC-EMPTY(1.2))/(FULL-EMPTY(?))*100
	// }
	// else{
	//	 FGAdjustSOC = ((FGPureSOC - 80)*10000)/9430; // (FGPureSOC-EMPTY(1.2))/(FULL-EMPTY(?))*100
	// }
...
#if defined(CONFIG_S5PC110_KEPLER_BOARD) || (defined CONFIG_S5PC110_T959_BOARD) || defined(CONFIG_S5PC110_SIDEKICK_BOARD) || (defined CONFIG_S5PC110_HAWK_BOARD) || (defined CONFIG_S5PC110_VIBRANTPLUS_BOARD)|| defined(CONFIG_S5PC110_FLEMING_BOARD) 
...
// ]] junghyunseok edit for exception code 20100511
	...
	#elif (defined(CONFIG_S5PC110_HAWK_BOARD) || defined(CONFIG_S5PC110_SIDEKICK_BOARD))
		switch (mode) {
			case FUEL_INT_1ST:
				rst_cmd[0] = 0xB0;
				rst_cmd[1] = 0x10; // 15%
