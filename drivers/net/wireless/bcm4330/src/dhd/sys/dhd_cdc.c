 ...
 *
 * $Id: dhd_cdc.c,v 1.51.6.24 2010/12/23 04:59:46 Exp $
 *
...
dhd_wlfc_enque_sendq(void* state, int prec, void* p)
{
	...
	if (NULL == WLFC_PKTQ_PENQ(&ctx->SENDQ, prec, p)) {
...
dhd_wlfc_init(dhd_pub_t *dhd)
{
	...
	uint32 tlv = dhd->wlfc_disabled? 0 :
	...
	if (dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0) < 0) {
		...
		*/
		DHD_ERROR(("dhd_wlfc_init(): successfully %s bdcv2 tlv signaling, %d\n",
			dhd->wlfc_disabled?"disabled":"enabled", tlv));
	}
	return BCME_OK;
}
...
dhd_wlfc_enable(dhd_pub_t *dhd)
{
	...
	if (dhd->wlfc_disabled || dhd->wlfc_state)
		return BCME_OK;
...
dhd_prot_hdrpull(dhd_pub_t *dhd, int *ifidx, void *pktbuf)
{
	...
	if (PKTLEN(dhd->osh, pktbuf) < (uint32) (BDC_HEADER_LEN + (h->dataOffset * 4))) {
		DHD_ERROR(("%s: rx data too short (%d < %d)\n", __FUNCTION__,
		           PKTLEN(dhd->osh, pktbuf), (BDC_HEADER_LEN + (h->dataOffset * 4))));
		return BCME_ERROR;
	}
...
#endif /* PROP_TXSTATUS */
	PKTPULL(dhd->osh, pktbuf, (BDC_HEADER_LEN + (h->dataOffset * 4)));
#endif /* BDC */
...
#if defined(USE_KEEP_ALIVE)
	dhd_enable_keepalive(dhd, 60000); /* 60 sec */
#endif
