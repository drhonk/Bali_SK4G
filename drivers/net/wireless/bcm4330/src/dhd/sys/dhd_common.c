 ...
 *
 * $Id: dhd_common.c,v 1.57.2.17 2010/12/23 01:54:50 Exp $
 */
#include <typedefs.h>
#include <osl.h>
...
dhd_wl_ioctl(dhd_pub_t *dhd_pub, int ifindex, wl_ioctl_t *ioc, void *buf, int len)
{
	...
	return dhd_prot_ioctl(dhd_pub, ifindex, ioc, buf, len);
}
            ...
            void *params, int plen, void *arg, int len, int val_size)
{
	...
	case IOV_GVAL(IOV_PROPTXSTATUS_ENABLE):
		int_val = dhd_pub->wlfc_disabled? 0 : 1;
		bcopy(&int_val, arg, val_size);
		break;

	case IOV_SVAL(IOV_PROPTXSTATUS_ENABLE):
		dhd_pub->wlfc_disabled = int_val? 0 : 1;
		break;
...
dhd_pktfilter_offload_enable(dhd_pub_t * dhd, char *arg, int enable, int master_mode)
{
	...
	rc = dhdcdc_set_ioctl(dhd, 0, WLC_SET_VAR, buf, buf_len, TRUE);
	rc = rc >= 0 ? 0 : rc;
	if (rc)
	...
	rc = dhdcdc_set_ioctl(dhd, 0, WLC_SET_VAR, buf, sizeof(buf), TRUE);
	rc = rc >= 0 ? 0 : rc;
	if (rc)
		DHD_TRACE(("%s: failed to add pktfilter %s, retcode = %d\n",
		__FUNCTION__, arg, rc));
