 ...
 *
 * $Id: dhd_linux.c,v 1.131.2.37 2010/12/24 01:40:57 Exp $
 */
...
#define HTSF_MINLEN 80     /* min. packet length to timestamp */
...
#if defined(OOB_INTR_ONLY)
#if 0
	/* Host registration for OOB dummy interrupt */
	if (bcmsdh_register_oob_dummy_intr(dhdp)) {
		del_timer(&dhd->timer);
		dhd->wd_timer_valid = FALSE;
		DHD_ERROR(("%s Host failed to resgister for OOB dummy\n", __FUNCTION__));
		return -ENODEV;
	}
...
void dhd_htsf_addtxts(dhd_pub_t *dhdp, void *pktbuf)
{
	...
	struct sk_buff *skb;
	uint32 htsf = 0;
	uint16 dport = 0, htsfmagic = 0xACAC;
	char *p1;
...
/*	if (proto == 17 && dport == tsport) { */
	if (dport == tsport) {
		...
		memcpy(skb->data+66, &htsfmagic, 2);
		memcpy(skb->data+68, &htsf, 4);
	}
}
...
void dhd_htsf_addrxts(dhd_pub_t *dhdp, void *pktbuf)
{
	...
	if (PKTLEN(osh, pktbuf) > HTSF_MINLEN)
		memcpy(&magic, p1+62, 2);

	if (magic == 0xACAC) {

		tspktcnt++;
		memcpy(&ts[tsidx].t1, skb->data+64, 12);

		ts[tsidx].t4 = dhd_get_htsf(dhd, 0);
