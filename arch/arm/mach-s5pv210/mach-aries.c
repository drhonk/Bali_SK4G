...
#ifdef CONFIG_VIDEO_SR030PC30 /*NAGSM_Android_HQ_Camera_SungkooLee_20100914*/
#include <media/SR030pc30_platform.h> 
#endif
#ifdef CONFIG_VIDEO_MC7 
...
#include <mach/gpio.h>
	...
	#if (defined(CONFIG_SIDEKICK_VER_B0) || defined(CONFIG_SIDEKICK_VER_B1))
	#include <mach/gpio-sidekick-setting-REV00.h>
	#elif defined(CONFIG_SIDEKICK_VER_B2)
	#include <mach/gpio-sidekick-setting-REV02.h>
	#else
	#include <mach/gpio-sidekick-setting-REV02.h>
	#endif
#else
...
//NAGSM_Android_HQ_Camera_SoojinKim : VSUB_1.1V & MIPI_1.1V
#if defined(CONFIG_USB_SUPPORT) || defined(CONFIG_VIDEO_TV20)
int ldo38_onoff_state = 0;
#define LDO38_USB_BIT    (0x1)
#define LDO38_TVOUT_BIT  (0x2)

#if defined(CONFIG_SIDEKICK_VER_B1)
int ldo3_mipi_onoff_state = 0;
#endif
#endif
...
static struct s3c2410_uartcfg smdkv210_uartcfgs[] __initdata = {
	...
	[1] = {
		...
		.ucon		= S5PV210_UCON_DEFAULT,
		.ulcon		= S5PV210_ULCON_DEFAULT,
#if defined(CONFIG_S5PC110_T959_BOARD) || defined(CONFIG_S5PC110_SIDEKICK_BOARD)
...
#if defined(CONFIG_S5PC110_SIDEKICK_BOARD)
//OFM NAGSM_Android_SEL_Kernel_Aakash_20100904
...
#if defined(CONFIG_S5PC110_SIDEKICK_BOARD)
		{ MAX8998_LDO16, &max8998_ldo16_data },	//OFM NAGSM_Android_SEL_Kernel_Aakash_20100904
#endif
...
static void lcd_cfg_gpio(struct platform_device *pdev)
{
	...
	/* DISPLAY_CS */
	if(HWREV == 0x0b){
		s3c_gpio_cfgpin(GPIO_DISPLAY_CS_L, GPIO_DISPLAY_CS_L_AF);
		s3c_gpio_setpull(GPIO_DISPLAY_CS_L, S3C_GPIO_PULL_NONE);
	}else{
		s3c_gpio_cfgpin(GPIO_DISPLAY_CS_H, GPIO_DISPLAY_CS_H_AF);
		s3c_gpio_setpull(GPIO_DISPLAY_CS_H, S3C_GPIO_PULL_NONE);
	}
	/* DISPLAY_CLK */
...
void lcd_cfg_gpio_early_suspend(void)
{
	...
	/* DISPLAY_CS */
	if(HWREV == 0x0b){
		s3c_gpio_cfgpin(GPIO_DISPLAY_CS_L, S3C_GPIO_OUTPUT);
		s3c_gpio_setpull(GPIO_DISPLAY_CS_L, S3C_GPIO_PULL_NONE);
		s3c_gpio_setpin(GPIO_DISPLAY_CS_L, 0);
	}else{
		s3c_gpio_cfgpin(GPIO_DISPLAY_CS_H, S3C_GPIO_OUTPUT);
		s3c_gpio_setpull(GPIO_DISPLAY_CS_H, S3C_GPIO_PULL_NONE);
		s3c_gpio_setpin(GPIO_DISPLAY_CS_H, 0);
	}
	/* DISPLAY_CLK */
...
void lcd_cfg_gpio_late_resume(void)
{
  ...
  /* DISPLAY_CS */
  	if(HWREV == 0x0b){
		s3c_gpio_cfgpin(GPIO_DISPLAY_CS_L, S3C_GPIO_OUTPUT);
		s3c_gpio_setpull(GPIO_DISPLAY_CS_L, S3C_GPIO_PULL_NONE);
		s3c_gpio_setpin(GPIO_DISPLAY_CS_L, 1);
  	}else{
		s3c_gpio_cfgpin(GPIO_DISPLAY_CS_H, S3C_GPIO_OUTPUT);
		s3c_gpio_setpull(GPIO_DISPLAY_CS_H, S3C_GPIO_PULL_NONE);
		s3c_gpio_setpin(GPIO_DISPLAY_CS_H, 1);
  	}
  #endif
}
...
static struct s3c_platform_fb s3c_lcd_data __initdata = {
        .hw_ver = 0x62,
        .clk_name = "sclk_fimd",
        .nr_wins = 5,
...
#if defined(CONFIG_FB_S3C_UPD161224)
static struct spi_gpio_platform_data uPD161224_spi_gpio_data = {
        .sck    = DISPLAY_CLK,
        .mosi   = DISPLAY_SI,
        .miso   = DISPLAY_SO,
        .num_chipselect = 2,
...
static struct platform_device s3c_device_spi_gpio = {
        ...
        .dev    = {
                ...
                #if defined(CONFIG_FB_S3C_UPD161224)                
                .platform_data  = &uPD161224_spi_gpio_data,
                #else
...
static	struct	i2c_gpio_platform_data	i2c14_platdata = {
	.sda_pin		= GPIO_OJ_SDA_28V,	//OJ_SDA_2.8V	No macros defined so direct using :(
	.scl_pin		= GPIO_OJ_SCL_28V,	//OJ_SCL_2.8V
	.udelay			= 1,	/* 250KHz */		
	.sda_is_open_drain	= 0,
	.scl_is_open_drain	= 0,
	.scl_is_output_only	= 0,
...
//NAGSM_Android_HQ_Camera_SungkooLee_20100914 VT camera
#if defined(CAMERA_GPIO_I2C)
static  struct  i2c_gpio_platform_data  i2c15_platdata = {
        .sda_pin                = GPIO_VTCAM_SDA_18V,
        .scl_pin                = GPIO_VTCAM_SCL_18V,
        .udelay                 = 2,    /* 250KHz */
        .sda_is_open_drain      = 0,
        .scl_is_open_drain      = 0,
        .scl_is_output_only     = 0,
...
/*NAGSM_Android_HQ_Camera_SungkooLee_20100914*/
...
static int SR030pc30_cam_stdby(bool onoff)
{
	int err;
	/* CAM_VGA_nSTBY - GPC1(0) */
	err = gpio_request(S5PV210_GPC1(0), "GPC1(0)");
	if (err) {
		printk(KERN_ERR "failed to request GPC1(0) for camera control\n");
		return err;
	}

	gpio_direction_output(S5PV210_GPC1(0), 0); 
	msleep(1);
...
static int SR030pc30_cam_nrst(bool onoff)
{
	int err;

	/* CAM_VGA_nRST - GPC1(4)*/
	err = gpio_request(S5PV210_GPC1(4), "GPC1(4)");
	if (err) {
		printk(KERN_ERR "failed to request GPC1(4) for camera control\n");
		return err;
	}
...
#if defined(ADD_5M_CAM)
static void mc7_mega_cam_en(bool onoff)
{
	int err;
...
#if defined (CONFIG_SIDEKICK_VER_B1)
static int SR030pc30_power_on()
{
	int err;

	printk(KERN_DEBUG "SR030pc30_power_on : mach\n");
...
static int SR030pc30_power_off()
{
	int err;

	printk(KERN_DEBUG "SR030pc30_power_off : mach\n");
...
static int SR030pc30_power_en(int onoff)
{
	...
	if(onoff){
		SR030pc30_ldo_en(true);
		s3c_gpio_cfgpin(S5PV210_GPE1(3), S5PV210_GPE1_3_CAM_A_CLKOUT);
		SR030pc30_cam_stdby(true);
		SR030pc30_cam_nrst(true);
		mdelay(100);
	} else {
		SR030pc30_cam_stdby(false);
		SR030pc30_cam_nrst(false);
		s3c_gpio_cfgpin(S5PV210_GPE1(3), 0);
		SR030pc30_ldo_en(false);
	}
...
#if !defined(CAMERA_GPIO_I2C) // for 0.1
...
#endif //end CONFIG_VIDEO_SR030PC30
...
{	
	...
	printk(KERN_DEBUG "mc7_power_on (mach-aries.c )\n");
...
static int mc7_power_off(void)
{
	...
	printk(KERN_DEBUG "mc7_power_off (mach-aries.c )\n");
...
/* External camera module setting */
static struct mc7_platform_data mc7_plat = {
	.default_width = 640,
	.default_height = 480,
	.pixelformat = V4L2_PIX_FMT_UYVY,
	.freq = 24000000,
...
static struct i2c_board_info  mc7_i2c_info = {
	I2C_BOARD_INFO("MC7", 0x3E>>1), //CHECK
	.platform_data = &mc7_plat,
};

static struct s3c_platform_camera mc7 = {
	.id		= CAMERA_PAR_A,
	.type		= CAM_TYPE_MIPI,
	.fmt		= ITU_601_YCBCR422_8BIT,
	.order422	= CAM_ORDER422_8BIT_CBYCRY,
	...
	.mipi_lanes	= 2,
	.mipi_settle	= 6, //CHECK timing
	.mipi_align	= 32, //CHECK bandwidth
...
static int status_gpio_blink_set(unsigned gpio, unsigned long * delay_on, unsigned long* delay_off)
{
	return 0;
}

static struct gpio_led_platform_data status_led = {
	.num_leds = ARRAY_SIZE(status_leds),
	.leds = status_leds,
	.gpio_blink_set=status_gpio_blink_set,
};
...
static struct platform_device status_led_dev = {
	...
	.id            = -1,
	.dev = {
		.platform_data = &status_led,
	},
};

static void __init status_led_register(void)
{
	int ret = platform_device_register(&status_led_dev);
	if (ret)
		printk(KERN_ERR "smdk: failed to register status_led device: %d\n", ret);
}
...
static struct i2c_board_info i2c_devs13[] __initdata = {
	{
#if defined(CONFIG_S5PC110_SIDEKICK_BOARD)
		I2C_BOARD_INFO("A1026_driver", (0x3E >> 1)),
#else
...
static struct platform_device *smdkc110_devices[] __initdata = {
	...
	&hall_device_slider,	//HALL IC NAGSM_Android_SEL_Kernel_Aakash_20100908
...
#if defined(CONFIG_S5PC110_SIDEKICK_BOARD)
	&s3c_device_i2c14,	//OFM NAGSM_Android_SEL_Kernel_Aakash_20100904
#else
    &s3c_device_i2c13,  // hdlnc_bp_ytkwon : 20100301
#endif

#if defined(CAMERA_GPIO_I2C)/*NAGSM_Android_HQ_Camera_SungkooLee_20100914*/
&s3c_device_i2c15, 
#endif
...
void confirm_to_gpio_initial(void)
{
	printk("%s : Initial gpio setting confirmed. \n",__func__);
}
EXPORT_SYMBOL(confirm_to_gpio_initial);
#endif
...
//hdlnc_ldj_20100823 
#endif
#if defined(CONFIG_S5PC110_SIDEKICK_BOARD)
	i2c_register_board_info(14, i2c_devs14, ARRAY_SIZE(i2c_devs14)); /* OFM NAGSM_Android_SEL_Kernel_Aakash_20100904 */
#endif
#ifdef CONFIG_FB_S3C_LTE480WV
	...
	s3c_ts_set_platdata(&s3c_ts_platform);
  	...
  	s3cfb_set_platdata(&s3c_lcd_data);
#endif
...
static int ldo38_control(int module, bool onoff)
{
    ...
    if (onoff)
    {
        ...
        {
            #if defined(CONFIG_SIDEKICK_VER_B1)
            if(!ldo3_mipi_onoff_state) //NAGSM_Android_HQ_Camera_SoojinKim : VSUB_1.1V & MIPI_1.1V
            #endif
            {
	            Set_MAX8998_PM_REG(ELDO3, 1);
	            msleep(1);
...
#elif defined CONFIG_S5PC110_SIDEKICK_BOARD	// SIDEKICK_KEYPAD
...
void s3c_setup_keypad_cfg_gpio(int rows, int columns)
{
	...
	for (gpio = S5PV210_GPJ1(5); gpio <= end; gpio++) {
		s3c_gpio_cfgpin(gpio, S3C_GPIO_SFN(3));
		s3c_gpio_setpull(gpio, S3C_GPIO_PULL_NONE);
	}
}
...
//For Starting/Stopping Tethering service
	...
	mdelay(300);  //wait for being ready for enabling wlan
}
