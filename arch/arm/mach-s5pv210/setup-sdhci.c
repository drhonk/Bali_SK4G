...
#if 0	//defined (CONFIG_MACH_S5PC110_ARIES)
...
//[NAGSM_Android_HDLNC_SDcard_Seojw_20110115 :  edit high speed clock timing				
...
/*
	printk(" Universal :SD Detect function \n");
	printk("eint conf %x  eint filter conf %x",readl(S5PV210_EINT1CON),
			readl(S5PV210_EINT1FLTCON0));
	printk("eint pend %x  eint mask %x",readl(S5PV210_EINTPEND(9)),
			readl(S5PV210_EINTMASK(9)));
*/
#endif
...
#else
//[NAGSM_Android_HDLNC_SDcard_SeoJaeWoong_20101104 : rev02 change T_FLASH_DETECT Pin 
#if defined(CONFIG_SIDEKICK_VER_B0)|| defined(CONFIG_SIDEKICK_VER_B1)
	card_status = (readl(S5PV210_GPH1DAT)) & (1 << 1);
#else
#if defined(CONFIG_SIDEKICK_VER_B2)	
	card_status = (readl(S5PV210_GPH1DAT)) & (1 << 2);
#else
//[NAGSM_Android_HDLNC_SDcard_SeoJaeWoong_20101104 : rev02 change T_FLASH_DETECT Pin
	card_status = (readl(S5PV210_GPH3DAT)) & (1 << 4);
#endif
...
void universal_sdhci2_cfg_ext_cd(void)
{
	...
	set_irq_type(IRQ_EINT(28), IRQ_TYPE_EDGE_BOTH);
		...
		set_irq_type(GPIO_T_FLASH_DETECT_IRQ, IRQ_TYPE_EDGE_BOTH);
#else
		s3c_gpio_setpull(S5PV210_GPH3(4), S3C_GPIO_PULL_NONE);
	set_irq_type(IRQ_EINT(28), IRQ_TYPE_EDGE_BOTH);
#endif
...
#if defined (CONFIG_MACH_S5PC110_ARIES) || defined(CONFIG_MACH_S5PC110_P1) || (CONFIG_MACH_S5PC110_JUPITER)
#if defined(CONFIG_S5PC110_SIDEKICK_BOARD)
	.ext_cd = GPIO_T_FLASH_DETECT_IRQ,
#else
	.ext_cd = IRQ_EINT(28),
#endif
