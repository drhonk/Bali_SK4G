...
extern int current_hall_ic_state;
...
static int s3c_pm_enter(suspend_state_t state)
{
	...
	*/
...
#if (defined(CONFIG_OPTICAL_GP2A) || defined(CONFIG_OPTICAL_TAOS_TRITON))
	...
	s3c_pm_set_eint(10, 0x4);   // T_FLASH_DETECT
	...
	if(current_hall_ic_state){
		printk("[Hall IC] - Low level....\n");
		s3c_pm_set_eint(13, 0x0); // Hall INT -> Low level
	}else{
		printk("[Hall IC] - High level....\n");
		s3c_pm_set_eint(13, 0x1); // Hall INT -> High level
	}
	
	if((is_calling_or_playing & IS_VOICE_CALL_2G) || (is_calling_or_playing & IS_VOICE_CALL_3G)){		
		s3c_pm_set_eint(14, 0x4); // volume down
		s3c_pm_set_eint(16, 0x4); // volume up
	}
	
	s3c_pm_set_eint(17, 0x4); //sendend 2.5
	s3c_pm_set_eint(18, 0x4); //sendend 3.5
	s3c_pm_set_eint(19, 0x2); // fuel_int
	s3c_pm_set_eint(20, 0x3); // wifi
	s3c_pm_set_eint(21, 0x4); // bt
	s3c_pm_set_eint(22, 0x2); // power key
	s3c_pm_set_eint(23, 0x2);   // Jack_nINT
	s3c_pm_set_eint(31, 0x4);   // OK key
